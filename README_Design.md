...
1. **Candidate**: register → log in → browse jobs → apply and upload resume → check application progress / assessment → start or continue the AI chat
2. **HR**: log in → create / modify / close job postings → view applications and assessments → set `hr_decision` & `hr_comments` → view the entire chat history
...
| PATCH  | `/jobs/{id}`          | JWT (HR)    | Update title / description / requirements / closing date / status |
...
The requirements, database schemas, and APIs above cover the entire business domain: registration & login, job management, application workflow, AI chat & scoring, HR decisions, and candidate self-service progress tracking.  
JWTs have no refresh/revocation mechanism, and services expose only the minimal internal APIs needed to complete this robust yet minimal feature loop. 