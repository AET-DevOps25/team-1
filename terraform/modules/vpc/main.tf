...
resource "aws_eip" "nat" {
  count = length(var.public_subnet_cidrs) # Create one NAT Gateway per public subnet/AZ for redundancy
...
resource "aws_route_table" "private" {
  ...
  route {
    cidr_block     = "0.0.0.0/0"
    nat_gateway_id = aws_nat_gateway.main[count.index].id # Route to corresponding NAT Gateway
  }
